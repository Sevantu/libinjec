...
func (i *Inject) NewConnection(proxyHost string, proxyPort int, request map[string]string) (net.Conn, error) {
	...
	if proxyHost == request["host"] && strconv.Itoa(proxyPort) == request["port"] {
		logConnecting = fmt.Sprintf("SEVANTU4G  Connecting to %s port %s", request["host"], request["port"])

	} else {
		logConnecting = fmt.Sprintf(
			"SEVANTU4G  Connecting to %s port %d -> %s port %s", proxyHost, proxyPort, request["host"], request["port"],
		)
	}
